...
/// 把 Ollama 对话转换为 <c>/v1/responses</c> 的 input/item 形状，并把 Responses SSE 或非流式响应
/// 归一化为现有下游可消费的 OpenAI Chat Completions 形状。
/// </summary>
public sealed class ResponsesUpstreamAdapter : IUpstreamAdapter
{
    ...
    /// <summary>从 Responses message.content 中提取 output_text/refusal 文本。</summary>
    private static void AppendMessageContent(Object? value, StringBuilder target)
    {
        foreach (var item in AsList(value))
        {
            ...
            if (type is "output_text" or "refusal") target.Append(part.Val("text")?.ToString());
        }
    }
    ...
    /// <summary>把动态 JSON 数组安全转换为可枚举列表。</summary>
